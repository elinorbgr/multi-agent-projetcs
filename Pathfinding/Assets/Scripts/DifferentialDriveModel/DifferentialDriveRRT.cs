...
public class DifferentialDriveRRT : MonoBehaviour {
	...
	static SteerResult steer(Vector3 start, Vector3 goal, float angle, float velocity, float maxSpeed, float maxRotSpeed, float length) {
		...
		while ((start-goal).magnitude > 1 || cost < 3) {
			...
			Vector2 u = DynamicCarMotionModel.computeU(start, goal, forward, velocity, length, maxSpeed, maxRotSpeed);
			angle += maxRotSpeed * step;
			velocity += u.x * step;
			start = nextpos;
	...
	static public RTTTree<Vector2> MoveOrder(Vector3 start, Vector3 goal, Vector3 forward, float velocity, float maxSpeed, float maxRotSpeed, float length, float minx, float miny, float maxx, float maxy) {
