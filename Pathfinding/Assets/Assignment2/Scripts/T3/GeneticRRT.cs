...
public class GeneticRRT : MonoBehaviour, ISynchroStart {
    ...
    class Individual {
        ...
        public Individual(List<List<Vector3>> genom, GeneticRRT simulator) {
            this.genom = genom;
    ...
    public List<List<Vector3>> generic_rrt(List<List<Vector3>> mobiles_clients) {
        ...
        for(int i = 0; i<mobiles_clients.Count; i++) {
            ...
            foreach(Vector3 client in mobiles_clients[i]) {
                RRTTree<Vector3> wps = DynamicRRTPathPlanning.MoveOrder(
                    start,
                    client,
                    acceleration,
                    minx,
                ...
                List<Vector3> nds = wps.cheapestInRadius(client,25).pathFromRoot();
